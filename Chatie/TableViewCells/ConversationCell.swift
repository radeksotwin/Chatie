...
class ConversationCell: UITableViewCell {
    ...
    var conversationViewModel: ConversationViewModel! {
        didSet {
            configure(with: conversationViewModel)
        }
    }
    ...
    public func configure(with viewModel: ConversationViewModel) {
        guard let currentUserName = UserDefaults.standard.value(forKey: "name") as? String else { return }
        
        userNameLabel.text = viewModel.name
        
        if viewModel.latestMessage.senderName == currentUserName {
            messageLabel.text = "You: " + "\(viewModel.latestMessage.text)"
        } else {
            messageLabel.text = viewModel.latestMessage.text
        }
        
        let path = "images/\(viewModel.otherUserEmail)_profile_picture.png"
