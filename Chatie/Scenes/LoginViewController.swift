...
final class LoginViewController: UIViewController, LoginPageConfiguration, AEAssessmentSessionDelegate, GIDSignInUIDelegate {
    ...
    func setRegistrationConfiguration() {
        ...
        UIView.animate(withDuration: 0.7, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
            guard let me = self else { return }
            me.logoImageView.shake()
            me.fbLoginButton.transform = CGAffineTransform(translationX: 525, y: 0)
            me.bgView.transform = CGAffineTransform(rotationAngle: 80)
            me.bgView.transform = CGAffineTransform(scaleX: 1.2, y: 1.2)
            me.bgView.layer.shadowRadius = 25
            me.logoImageView.tintColor = .white
            me.firstNameTextField.transform = CGAffineTransform(translationX: dynamicXPositionValue, y: 0)
            me.view.layoutIfNeeded()
        }
        
        UIView.animate(withDuration: 0.4, delay: 0.4, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
            guard let me = self else { return }
            me.loginButton.transform = CGAffineTransform(translationX: 0, y: 60)
            me.loginInfoLabel.alpha = 1
            me.registrationInfoLabel.alpha = 0
            me.view.layoutIfNeeded()
        } completion: { _ in
            UIView.animate(withDuration: 0.6, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
                guard let me = self else { return }
                me.lastNameTextField.transform = CGAffineTransform(translationX: dynamicXPositionValue, y: 0)
                me.view.layoutIfNeeded()
            }
        }
        
        UIView.animate(withDuration: 0.5, delay: 1.2, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseOut) { [weak self] in
            guard let me = self else { return }
            me.loginButton.transform = CGAffineTransform(translationX: 500, y: 60)
            me.registerButton.transform = CGAffineTransform(translationX: dynamicXPositionValue, y: 0)
            me.view.layoutIfNeeded()
        } completion: { _ in
            UIView.animate(withDuration: 0.7, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
                guard let me = self else { return }
                me.bgView.transform = CGAffineTransform(rotationAngle: 0)
                me.bgView.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
                me.registerButton.transform = CGAffineTransform(translationX: dynamicXPositionValue, y: -55)
                me.bgView.layer.shadowRadius = 17
                me.segmentedView.isEnabled = true
                me.logoImageView.tintColor = .black
                me.view.layoutIfNeeded()
            }
        }
        isAnimating = false
    ...
    func setLoginConfiguration() {
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 5, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
            guard let me = self else { return }
            me.logoImageView.shake()
            me.bgView.transform = CGAffineTransform(rotationAngle: 80)
            me.bgView.transform = CGAffineTransform(scaleX: 1.2, y: 1.2)
            me.firstNameTextField.transform = CGAffineTransform(translationX: -centerXValue, y: 0)
            me.registerButton.transform = CGAffineTransform(translationX: centerXValue, y: 0)
            me.logoImageView.tintColor = .white
            me.view.layoutIfNeeded()
        }
        
        UIView.animate(withDuration: 0.4, delay: 0.4, usingSpringWithDamping: 5, initialSpringVelocity: 0.1, options: .curveEaseOut) { [weak self] in
            guard let me = self else { return }
            me.registerButton.transform = CGAffineTransform(translationX: -525, y: 0)
            me.loginButton.transform = CGAffineTransform(translationX: 0, y: 60)
        } completion: { _ in
            UIView.animate(withDuration: 0.5, delay: 0.3, usingSpringWithDamping: 5, initialSpringVelocity: 0.1, options: .curveEaseInOut) {
                [weak self] in
                guard let me = self else { return }
                me.loginButton.transform = CGAffineTransform(translationX: 0, y: 0)
                me.loginInfoLabel.alpha = 0
                me.registrationInfoLabel.alpha = 1
                me.view.layoutIfNeeded()
            }
        }
        
        UIView.animate(withDuration: 0.4, delay: 1.2, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
            guard let me = self else { return }
            me.lastNameTextField.transform = CGAffineTransform(translationX: -centerXValue, y: 0)
            me.view.layoutIfNeeded()
        } completion: { _ in
            UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 5, initialSpringVelocity: 0.1, options: .curveEaseInOut, animations: {
                ...
                UIView.animate(withDuration: 0.7, delay: 0.0, usingSpringWithDamping: 5, initialSpringVelocity: 0.1, options: .curveEaseInOut) { [weak self] in
                    guard let me = self else { return }
                    me.bgView.transform = CGAffineTransform(rotationAngle: 60)
                    me.bgView.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
                    me.registerButton.transform = CGAffineTransform(translationX: 0, y: 0)
                    me.loginButton.transform = CGAffineTransform(translationX: 0, y: 0)
                    me.fbLoginButton.transform = CGAffineTransform(translationX: 0, y: 0)
                    me.logoImageView.tintColor = .black
                    me.segmentedView.isEnabled = true
                    me.view.layoutIfNeeded()
                }
