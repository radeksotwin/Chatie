...
class ConverastionsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let tableView: UITableView = {
        ...
        tableView.register(ConversationCell.self, forCellReuseIdentifier: ConversationCell.cellIdentifier)
    ...
    private func startListeningForConversations() {
        ...
        DatabaseManager.shared.getAllConversations(for: safeEmail, completion: { [weak self] result in
            ...
            case .success(let conversations):
                ...
                self?.conversationViewModels = conversations.map({ return ConversationViewModel(conversation: $0) })
...
//        })
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let conversationViewModel = conversationViewModels[indexPath.row]
        let cell = tableView.dequeueReusableCell(withIdentifier: "chatCell", for: indexPath) as! ConversationCell
        
        cell.setupView()
        cell.conversationViewModel = conversationViewModel
...
//        tableView.reloadRows(at: [indexPath], with: .fade)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return conversationViewModels.count
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let conversationId = conversationViewModels[indexPath.row].id
            
            tableView.beginUpdates()
            
            self.conversationViewModels.remove(at: indexPath.row)
...
//        let atb = [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 17, weight: .semibold), .foregroundColor: UIColor.black], for: .selected)]
        ...
        openConversation(with: conversationViewModel)
    }
    
    func openConversation(with viewModel: ConversationViewModel) {
        let chatVc = ChatViewController(id: viewModel.id, with: viewModel.otherUserEmail)
        chatVc.navigationItem.largeTitleDisplayMode = .never
        chatVc.title = viewModel.name
...
//        navigationController?.navigationBar.topItem?.rightBarButtonItem?.tintColor = .white
    ...
    @objc func addChat() {
        ...
        vc.completion = { [weak self] result in
            ...
            let currentConversations = me.conversationViewModels
